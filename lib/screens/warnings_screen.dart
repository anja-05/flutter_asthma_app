    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: RefreshIndicator(
          ...
          child: _loading ? const Center(child: CircularProgressIndicator()) : _error != null
              ? _buildError()
              : ListView(
            ...
            children: [
              ...
              if (aqi > 60) WarningBanner(
                ...
                message: aqi > 80 ? 'Kein Sport im Freien.' : 'Vorsicht bei Empfindlichkeit.',
                severity: aqi > 80 ? WarningSeverity.danger : WarningSeverity.warning,
              ) else const WarningBanner(title: 'Gute Luft', message: 'Alles okay.', severity: WarningSeverity.info),
              const SizedBox(height: 16),
              AqiCard(aqiValue: aqi, category: aqi <= 40 ? 'Gut' : aqi <= 70 ? 'Mäßig' : 'Schlecht'),
              const SizedBox(height: 16),
              ...
              const SizedBox(height: 24),
              const Text('5-Tage-Vorhersage', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF388E3C))),
              const SizedBox(height: 12),
              if (_forecast == null || _forecast!.isEmpty) const Text('Keine Daten') else AppCard(child: Column(
                children: List.generate(_forecast!.length, (i) {
                  ...
                  return Container(
                    ...
                    decoration: i < _forecast!.length - 1 ? BoxDecoration(border: Border(bottom: BorderSide(color: Colors.grey.shade200))) : null,
                    child: Row(children: [
                      SizedBox(width: 85, child: Text(DateFormat('EEE, dd.MM.', 'de_DE').format(f['date']), style: const TextStyle(fontWeight: FontWeight.w500))),
                      const Spacer(),
                      _miniIcon(Icons.air, _getAqiColor(f['AQI']), 'AQI ${f['AQI']}'),
                      const SizedBox(width: 16),
                      _miniIcon(Icons.local_florist, _colors[r], _texts[r]),
                    ]),
                  );
                }),
              )),
